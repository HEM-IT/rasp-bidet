...
# Ctrl+C(SIGINT) 시 device status를 ready로 복구한 뒤 종료하기 위한 값 (main()에서 설정)
_exit_api_base = None
_exit_gas_id = None
...
def _sigint_handler(signum, frame):
    if _exit_api_base and _exit_gas_id:
        try:
            ...
            print("[gpio_controller] Ctrl+C: device status → ready 후 종료", file=sys.stderr)
        except Exception as e:
            print(f"[gpio_controller] Ctrl+C: device status ready 전송 실패: {e}", file=sys.stderr)
    sys.exit(0)
...
def main():
    ...
    global _exit_api_base, _exit_gas_id
    _exit_api_base = api_base
    _exit_gas_id = gas_id
    try:
